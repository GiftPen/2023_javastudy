...
public class Ex01_class {  //   <- 헤더
	...
	// 1. 헤더 : [접근 제한자] [클래스 종류에 대한 예약어] class 클래스이름{     }  // 여기서 대괄호 뜻은 생략가능하단 뜻
...
//					   첫글자 대문자 나머지는 소문자
	...
	//  2) 메서드이름 : 변수와 같음, 첫글자 소문자, 두 단어 이상일때 단어 첫글다들은 대문자
	//				 숫자는 중간이나 뒤에 사용, 공백은 사용 불가
	//  3) **인자 = 매개변수 : 보통은 실행할때 필요한 정보를 외부에서 받기 위한 통로
	//     				   생략 가능하고 여러개 사용 가능
	//  4) **오버로딩(중복정의) : 메서드 이름이 같으나 인자 수나 인자 자료형이 다르면 다른 메서드로 판단한다.
	//			           즉, 인자의 수나 자료형이 다르면 같은 이름의 메서드가 여러개 존재 할 수 있다.(인자가 달라야)
	//  5) **getter() : 호출하는 입장에서 메서드를 이용해서 정보를 가져오는 메서드.
	//	   **setter() : 호출하는 입장에서 메서드를 이용해서 정보를 변경하는 메서드.
	//	   변수가 private 이면 직접 접근을 할 수 없다. => getter()/setter()를 이용해서 접근하다.
	
	// 4. 생성자 : **클래스가 객체로 만들어질 때 '한번 호출' 된다.(반드시)
	//    1) 목적 : 멤버필드(변수, 상수)의 '초기값을 지정하는 것이 목적'이다.-> 역할 
	...
	//		 해당 클래스에 생성자가 없으면 기본 생성자가 생략되어 있음
	//		 해당 클래스의 생성자가 없으면 객체를 생성할 때 기본 생성자를 호출 한다.

	//	  3) 클래스를 이용해서 객체 만들때 생성자 호출
	//       Scanner       scan         =       new         Scanner(System.in); <-인자가 있는 생성자
	...
	//								     일반적인 클래스의 멤버필드나 멤버메서드는 모두 인스턴스 이다.
	//		 호출방법 :  객체 생성한 다음에 호출     
