...
public class Ex01_Menu extends JFrame {
	...
	public Ex01_Menu() {
		...
		font_form.addSeparator(); // 사이 선긋기
		font_form.add(i_item2);
		font_form.addSeparator(); // 사이 선긋기
		font_form.add(i_item3);

		m_help.add(i_help);
		m_help.addSeparator(); // 사이 선긋기
		m_help.add(i_info);
		...
		i_newFile.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (size == 0) {
					...
					int res = JOptionPane.showOptionDialog(getParent(), "변경된 내용을 제목 없음에 저장하시겠습니까?", "간단 메모장",
		...
		i_openFile.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				//System.out.println(openpath); // 취소하면 openpath가 nullnull값이 나옴
				if (!openpath.equals("nullnull")) { // 취소할때 반응을 안해야함
					jta.setText("");
