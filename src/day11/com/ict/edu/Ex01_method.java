...
public class Ex01_method {
	// 클래스 : 멤버필드, 멤버메서드, 생성자
	// 멤버필드 : 인스턴스변수, 인스턴수상수, static변수, static상수 
	...
	//    ** 반환형 : 반드시 호출한 곳으로 되돌아 가는데 실행 결과를 가지고 갈때 실행결과의 자료형을 반환형이라한다
	//				실행결과를 가지고 가지 않으면 void 라는 예약어를 사용한다.
	
	// instance 메서드
	public void getTotal() {
		total = kor + eng + math ;  // 호출하면 실행됬을때 값이 계산
		// return;(생략가능)
	}
	// instance 메서드  
	public int getTotal2() {
		...
		// 반환형이 void가 아니면 반드시 맨 마지막줄에 (return 데이터)나 ,데이터를 가지고 있는 변수
		return sum;
	}
	
	// instance 메서드  //이렇게도 가능
		public int getTotal3() {
		 
			// 반환형이 void가 아니면 반드시 맨 마지막줄에 (return 데이터)나 ,데이터를 가지고 있는 변수
			// 인스턴스는 스택틱 변수 사용 가능(스택틱은 미리 만들어 져있는 개념이라 사용 가능하고 반대는 불가능)
