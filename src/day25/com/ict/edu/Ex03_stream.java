...
public class Ex03_stream {
	...
	// 2. 문자 스트림 : 모든 처리를 2byte 처리
	//		   대상 : 세계 모든 문자로 구성된 파일을 입출력한다.
	//    최상위 클래스 : Reader(입력), Writer(출력)
	
	// 1,2 결합 스트림 : 바이트 스트림 => 문자 스트림 (스트림사이를 변경)
	...
	//    최상위 클래스 : ObjectInputStream 안에 readObject() 메서드 => 객체 역직렬화
	//				  ObjectOutputStream 안에 writerObject() 메서드가 있다 => 객체 직렬화 메서드
}
