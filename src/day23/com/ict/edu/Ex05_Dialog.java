	...
	public Ex05_Dialog() {
		...
		jb1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// JOptionPane.showMessageDialog(jp, "방가방가"); //INFORMATETION_MESSAGE(기본)
				// JOptionPane.showMessageDialog(getParent(), "방가방가"); // WARNING_MESSAGE
				
				// JOptionPane.showMessageDialog(parentComponent, message, title, messageType);
				// massageType : ERROR_MESSAGGE, INFORMATETION_MESSAGE(기본), WARNING_MESSAGE,
		...
		jb3.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
			// 입력 다이얼 로그 : 사용자에게 정보를 받는 다이얼 로그 (js(자바스크립트에서는 prompt()) // 왠만해선 잘 안쓰이는 것
		...
		jb4.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
			// 옵션 다이얼 로그 : 사용자 다이얼 로그 (사용자가 만드는 다이얼 로그) //앞에 배운것들이 다들어감
